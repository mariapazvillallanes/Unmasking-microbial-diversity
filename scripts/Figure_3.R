...
# ---- Read data ----
raw_data <- read.csv("data/Figure_3.csv", sep = ";", check.names = FALSE, row.names = 1)
...
# TOP20 géneros globales (suma por fila)
...
# Ceros a NA para mostrarlos en blanco
data_top20[data_top20 == 0] <- NA

# Valores mostrados como %
...
# Etiquetas de filas en cursiva
italic_labels <- parse(text = paste0("italic('", rownames(data_top20), "')"))

# ---- Export PDF (vectorial) ----
while (!is.null(dev.list())) dev.off()
CairoPDF("output/Figure_3.pdf", width = 12, height = 10, family = "Times New Roman")
...
# ---- Export TIFF (600 dpi) ----
tiff("output/Figure_3.tiff", width = 12, height = 10, units = "in", res = 600, compression = "lzw")
