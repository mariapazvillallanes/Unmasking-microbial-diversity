...
library(patchwork)
library(Cairo)
library(ggtext)   # para element_markdown en el eje X

# ---- Read & clean ----
df <- read.csv("data/Figure_9.csv", sep = ";", check.names = FALSE)
names(df) <- trimws(names(df))

# Abundance como numérico (coma -> punto)
df$Abundance <- as.numeric(gsub(",", ".", df$Abundance))

# Normaliza nombres esperados por si vienen con variantes
# Esperado: Treatment, Ecosystem, Class, Abundance
needed <- c("Treatment","Ecosystem","Class","Abundance")
missing <- setdiff(needed, names(df))
if (length(missing)) stop(paste("Faltan columnas:", paste(missing, collapse=", ")))

# ---- Mapear factores y etiquetas ----
df <- df %>%
  mutate(
    System = case_when(
...
# Si todo 0 en algún panel, evita NaN
...
final_plot <- ggplot(df_sum, aes(x = System, y = Relative_Abundance, fill = Class)) +
  ...
  labs(
    title = "Bacterial community structure by Domestication, Medium and Lifestyle",
    x = NULL,
...
# ---- Export ----
CairoPDF("output/Figure_9.pdf", width = 14, height = 8, family = "Times New Roman")
print(final_plot); dev.off()

ggsave("output/Figure_9.tiff", plot = final_plot,
