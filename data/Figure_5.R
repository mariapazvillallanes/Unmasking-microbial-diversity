# ---- Packages ----
library(ggplot2); library(ComplexUpset); library(patchwork)
library(ggtext); library(cowplot); library(showtext)

add_times <- function() {
  sys <- tolower(Sys.info()[["sysname"]])
  if (grepl("windows", sys)) {
    font_add(family = "Times New Roman", regular = "C:/Windows/Fonts/times.ttf")
  } else if (grepl("darwin", sys)) {
...
data <- read.csv("data/Figure_5.csv", sep = ";", check.names = TRUE)
data[is.na(data)] <- 0
...
ggsave("output/Figure_5.pdf", plot_final,
       device = cairo_pdf, width = 18, height = 12.5, units = "cm", dpi = 600, bg = "white")
ggsave("output/Figure_5.tiff", plot_final,
