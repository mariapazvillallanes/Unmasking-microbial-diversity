...
# ---- Read data ----
df <- read.csv("data/Figure_7.csv", sep = ";", stringsAsFactors = FALSE)

# Esperado: Sample, Treatment, Organ Material, Genera, Abundance
...
# ---- ANOVA + Tukey ----
anova_shannon <- aov(Shannon ~ Treatment, data = shannon_index)
tukey_tbl <- TukeyHSD(anova_shannon)$Treatment

# Comparaciones (solo las que existan en los datos)
...
# Orden de extracción de p-valores en Tukey
...
# Ajusta longitud por si falta alguna comparación
if (length(comparisons) != length(p_labels)) {
  # recorta o repite de forma segura
...
# ---- Export ----
CairoPDF("output/Figure_7a.pdf", width = 6, height = 5, family = "Times New Roman")
print(p); dev.off()

tiff("output/Figure_7a.tiff", units = "in", width = 6, height = 5,
